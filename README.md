...
- **Per-worktree docker compose stack** — each worktree gets its own Postgres, MinIO, Inngest, and Inbucket services on dynamic ports, isolated under the `crbn_<slug>` compose project (containers like `crbn_<slug>-postgres-1`). The shared host-wide stack uses project `crbn_shared` with fixed container names (`crbn_postgres`, `crbn_redis`, `crbn_minio`). Routing is handled by [portless](https://github.com/portless-dev/portless) (a local HTTPS reverse proxy that serves `*.dev` hostnames on `:443` with locally-trusted certs — installed automatically on first `crbn up`).
...
The backend stack (Postgres, MinIO, Inngest, and Inbucket) runs from `docker-compose.dev.yml` under a per-worktree compose project (`crbn_<slug>`), managed by `crbn up` / `down` / `reset`. Ports are allocated dynamically per worktree so multiple branches can run side-by-side. Key changes:
