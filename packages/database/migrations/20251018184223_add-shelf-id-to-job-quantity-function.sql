...
WITH
  ...
  open_jobs AS (
    SELECT
      jm."itemId" AS "jobItemId",
      SUM(jm."estimatedQuantity" - jm."quantityIssued") AS "quantityOnProductionOrder"
    FROM job_materials jm
    WHERE jm."methodType" = 'Make'
    GROUP BY jm."itemId"
  ),
  open_job_requirements AS (
    SELECT 
      ojr."itemId",
      ojr."shelfId",
      SUM(ojr."quantityOnProductionDemand") AS "quantityOnProductionDemand"
    FROM (
      SELECT 
        jm."itemId",
        jm."shelfId",
        jm."estimatedQuantity" - jm."quantityIssued" AS "quantityOnProductionDemand"
      FROM "jobMaterial" jm
    ...
    GROUP BY ojr."itemId", ojr."shelfId"
  ),
