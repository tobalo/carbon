...
import { Pool } from "pg";
import {
  closePostPurchaseInvoicePool,
  postPurchaseInvoice
} from "../src/post-purchase-invoice.ts";
...
const containerName = `carbon-pg18-post-purchase-invoice-smoke-${process.pid}`;
...
async function main() {
  ...
  try {
    ...
    await setupPostPurchaseInvoiceFixture(pool);
    ...
    console.log("Post-purchase-invoice smoke passed");
    console.log("- pg18-trixie Docker migration apply succeeded");
    console.log("- purchase invoice posting verified");
    console.log("- invoiced quantity rollback verified");
  } finally {
    await closePostPurchaseInvoicePool();
...
async function setupPostPurchaseInvoiceFixture(db: Pool) {
  await db.query(`
    INSERT INTO "user" (
    ...
    INSERT INTO "purchaseOrderLine" (
      ...
      "purchaseQuantity", "quantityInvoiced", "quantityReceived",
      "quantityToInvoice", "quantityToReceive", "purchaseUnitOfMeasureCode",
    ...
    VALUES (
      'pol1', 'po1', 'Part', 'item1', 'loc1', 5, 0, 0, 5, 5,
      'EA', 'EA', 1, 10, 10, 1, false, false, false, 1, 0, 0,
      'co1', NOW(), 'user1'
    );

    INSERT INTO "purchaseInvoice" (
      id, "invoiceId", status, "supplierId", "supplierInteractionId",
      "currencyCode", "locationId", balance, subtotal, "totalAmount",
      "totalDiscount", "totalTax", "exchangeRate", "customFields",
      "internalNotes", "companyId", "createdAt", "createdBy"
    )
    VALUES (
      'invoice1', 'PINV-1', 'Draft', 'supplier1', 'si1', 'USD', 'loc1',
      50, 50, 50, 0, 0, 1, '{}'::jsonb, '{}'::jsonb,
      'co1', NOW(), 'user1'
    );

    INSERT INTO "purchaseInvoiceLine" (
      id, "invoiceId", "invoiceLineType", "itemId", "locationId",
      quantity, "purchaseOrderId", "purchaseOrderLineId",
      "purchaseUnitOfMeasureCode", "inventoryUnitOfMeasureCode",
      "conversionFactor", "unitPrice", "supplierUnitPrice",
      "supplierExtendedPrice", "supplierShippingCost", "supplierTaxAmount",
      "taxPercent", "sortOrder", "exchangeRate", "customFields",
      "internalNotes", "companyId", "createdAt", "createdBy"
    )
    VALUES (
      'pil1', 'invoice1', 'Part', 'item1', 'loc1', 5, 'po1', 'pol1',
      'EA', 'EA', 1, 10, 10, 50, 0, 0, 0, 1, 1,
      '{}'::jsonb, '{}'::jsonb, 'co1', NOW(), 'user1'
    );
  `);
}
...
async function verifyPostedInvoice(db: Pool) {
  await assertText(
    db,
    `SELECT status FROM "purchaseInvoice" WHERE id = 'invoice1'`,
    "Open",
    "posted invoice status"
  );
  await assertCount(
    db,
    `SELECT count(*) FROM "purchaseInvoice" WHERE id = 'invoice1' AND "postingDate" IS NOT NULL`,
    1,
    "invoice posting date"
  );
  await assertQuantity(
    db,
    `SELECT "quantityInvoiced" AS quantity FROM "purchaseOrderLine" WHERE id = 'pol1'`,
    5,
    "invoiced purchase order quantity"
  );
  await assertText(
    db,
    `SELECT status FROM "purchaseOrder" WHERE id = 'po1'`,
    "To Receive",
    "purchase order status after invoice"
  );
}
