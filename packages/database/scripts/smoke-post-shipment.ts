...
import { Pool } from "pg";
import { closePostShipmentPool, postShipment } from "../src/post-shipment.ts";
...
const containerName = `carbon-pg18-post-shipment-smoke-${process.pid}`;
...
async function main() {
  ...
  try {
    ...
    await setupPostShipmentFixture(pool);
    await postShipment({
      type: "post",
    ...
    await verifyPostedShipment(pool);
    ...
    await verifyVoidedShipment(pool);

    console.log("Post-shipment smoke passed");
    console.log("- pg18-trixie Docker migration apply succeeded");
    console.log("- sales-order shipment posting verified");
    console.log("- item-ledger reversal and sent quantity rollback verified");
  } finally {
    await closePostShipmentPool();
...
async function setupPostShipmentFixture(db: Pool) {
  await db.query(`
    INSERT INTO "user" (
    ...
    INSERT INTO "salesOrderLine" (
      ...
      "saleQuantity", "quantitySent", "quantityToSend", "unitOfMeasureCode",
      "unitPrice", "methodType", "addOnCost", "nonTaxableAddOnCost",
      "shippingCost", "taxPercent", "sortOrder", "invoicedComplete",
      "sentComplete", "requiresInspection", status, "companyId", "createdAt",
      "createdBy"
    )
    VALUES (
      'sol1', 'so1', 'Part', 'item1', 'loc1', 5, 0, 5, 'EA',
      10, 'Pull from Inventory', 0, 0, 0, 0, 1, false, false,
      false, 'Ordered', 'co1', NOW(), 'user1'
    );

    INSERT INTO "shipment" (
      id, "shipmentId", status, "sourceDocument", "sourceDocumentId",
      "sourceDocumentReadableId", "locationId", "customerId", "companyId",
      "createdAt", "createdBy"
    )
    VALUES (
      'shipment1', 'SHIP-1', 'Pending', to_jsonb('Sales Order'::text),
      'so1', 'SO-1', 'loc1', 'customer1', 'co1', NOW(), 'user1'
    );

    INSERT INTO "shipmentLine" (
      id, "shipmentId", "lineId", "itemId", "locationId", "orderQuantity",
      "outstandingQuantity", "shippedQuantity", "requiresBatchTracking",
      "requiresSerialTracking", "unitOfMeasure", "unitPrice", "companyId",
      "createdAt", "createdBy"
    )
    VALUES (
      'sl1', 'shipment1', 'sol1', 'item1', 'loc1', 5, 5, 5,
      false, false, 'EA', 10, 'co1', NOW(), 'user1'
    );
  `);
}

async function verifyPostedShipment(db: Pool) {
  await assertText(
    db,
    `SELECT status FROM "shipment" WHERE id = 'shipment1'`,
    "Posted",
    "posted shipment status"
  );
  await assertText(
    db,
    `SELECT "postedBy" AS value FROM "shipment" WHERE id = 'shipment1'`,
    "user1",
    "posted by"
  );
  await assertQuantity(
    db,
    `SELECT "quantitySent" AS quantity FROM "salesOrderLine" WHERE id = 'sol1'`,
    5,
    "sent sales order quantity"
  );
  await assertText(
    db,
    `SELECT status FROM "salesOrder" WHERE id = 'so1'`,
    "To Invoice",
  ...
  await assertQuantity(
    ...
    `
      ...
      FROM "itemLedger"
      WHERE "documentId" = 'shipment1'
        AND "documentType" #>> '{}' = 'Sales Shipment'
    `,
    -5,
    "sales shipment item ledger quantity"
  );
}

async function verifyVoidedShipment(db: Pool) {
  await assertText(
    db,
    `SELECT status FROM "shipment" WHERE id = 'shipment1'`,
    "Voided",
    "voided shipment status"
  );
  await assertQuantity(
    db,
    `SELECT "quantitySent" AS quantity FROM "salesOrderLine" WHERE id = 'sol1'`,
    0,
    "voided sales order quantity"
  );
  await assertText(
    db,
    `SELECT status FROM "salesOrder" WHERE id = 'so1'`,
    "To Ship and Invoice",
    "sales order status after void"
  );
  await assertQuantity(
    db,
    `
    ...
    `,
    0,
    "voided sales shipment item ledger quantity"
  );
}
