...
import { Pool } from "pg";
import {
  closePostSalesInvoicePool,
  postSalesInvoice
} from "../src/post-sales-invoice.ts";
...
const containerName = `carbon-pg18-post-sales-invoice-smoke-${process.pid}`;
...
async function main() {
  ...
  try {
    ...
    await setupPostSalesInvoiceFixture(pool);
    ...
    await verifyPostedInvoice(pool);
    ...
    await verifyVoidedInvoice(pool);

    console.log("Post-sales-invoice smoke passed");
    console.log("- pg18-trixie Docker migration apply succeeded");
    console.log("- sales invoice posting verified");
    console.log("- invoiced quantity rollback verified");
  } finally {
    await closePostSalesInvoicePool();
...
async function setupPostSalesInvoiceFixture(db: Pool) {
  await db.query(`
    INSERT INTO "user" (
    ...
    INSERT INTO "salesOrderLine" (
      ...
      "saleQuantity", "quantityInvoiced", "quantityToInvoice",
      "quantitySent", "quantityToSend", "unitOfMeasureCode", "unitPrice",
    ...
    VALUES (
      'sol1', 'so1', 'Part', 'item1', 'loc1', 5, 0, 5, 0, 5, 'EA',
      10, 'Pull from Inventory', 0, 0, 0, 0, 1, false, false,
      false, 'Ordered', 'co1', NOW(), 'user1'
    );

    INSERT INTO "salesInvoice" (
      id, "invoiceId", status, "customerId", "currencyCode", "locationId",
      balance, subtotal, "totalAmount", "totalDiscount", "totalTax",
      "exchangeRate", "customFields", "externalNotes", "internalNotes",
      "companyId", "createdAt", "createdBy"
    )
    VALUES (
      'invoice1', 'INV-1', 'Draft', 'customer1', 'USD', 'loc1',
      50, 50, 50, 0, 0, 1, '{}'::jsonb, '{}'::jsonb, '{}'::jsonb,
      'co1', NOW(), 'user1'
    );

    INSERT INTO "salesInvoiceShipment" (
      id, "shippingCost", "customFields", "companyId", "createdAt", "createdBy"
    )
    VALUES ('invoice1', 0, '{}'::jsonb, 'co1', NOW(), 'user1');

    INSERT INTO "salesInvoiceLine" (
      id, "invoiceId", "invoiceLineType", "itemId", "locationId",
      quantity, "salesOrderId", "salesOrderLineId", "unitOfMeasureCode",
      "unitPrice", "methodType", "addOnCost", "nonTaxableAddOnCost",
      "setupPrice", "shippingCost", "taxPercent", "sortOrder",
      "exchangeRate", "customFields", "externalNotes", "internalNotes",
      "companyId", "createdAt", "createdBy"
    )
    VALUES (
      'sil1', 'invoice1', 'Part', 'item1', 'loc1', 5, 'so1', 'sol1',
      'EA', 10, 'Pull from Inventory', 0, 0, 0, 0, 0, 1, 1,
      '{}'::jsonb, '{}'::jsonb, '{}'::jsonb, 'co1', NOW(), 'user1'
    );
  `);
}

async function verifyPostedInvoice(db: Pool) {
  await assertText(
    db,
    `SELECT status FROM "salesInvoice" WHERE id = 'invoice1'`,
    "Submitted",
    "posted invoice status"
  );
  await assertCount(
    db,
    `SELECT count(*) FROM "salesInvoice" WHERE id = 'invoice1' AND "postingDate" IS NOT NULL`,
    1,
    "invoice posting date"
  );
  await assertQuantity(
    db,
    `SELECT "quantityInvoiced" AS quantity FROM "salesOrderLine" WHERE id = 'sol1'`,
    5,
    "invoiced sales order quantity"
  );
  await assertText(
    db,
    `SELECT status FROM "salesOrder" WHERE id = 'so1'`,
    "To Ship",
    "sales order status after invoice"
  );
}

async function verifyVoidedInvoice(db: Pool) {
  await assertText(
    db,
    `SELECT status FROM "salesInvoice" WHERE id = 'invoice1'`,
    "Voided",
